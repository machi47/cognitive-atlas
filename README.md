...
Cognitive Atlas is a private, local-first conversation-centered learning kernel. It is for messy associative thinking: the main chat stays compact and readable while the backend extracts topics, claims, source needs, open questions, map patches, and provenance into an inspectable atlas.
...
- Default LLM provider: fake/offline deterministic adapter
...
Fake mode is default. To try Codex:

```sh
ATLAS_LLM_PROVIDER=codex ./scripts/run_api.sh
```

If Codex is missing or unhealthy, the app falls back to fake mode instead of crashing.
...
- `ATLAS_DATA_DIR=./data`
- `ATLAS_LLM_PROVIDER=fake|codex|openai`
